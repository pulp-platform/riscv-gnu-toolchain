...
/* OPRECOMP Support functions for alternative half-precision format.  */

static void encode_oprecomp_half (const struct real_format *fmt,
				  long *, const REAL_VALUE_TYPE *);
static void decode_oprecomp_half (const struct real_format *,
				  REAL_VALUE_TYPE *, const long *);

static void
encode_oprecomp_half (const struct real_format *fmt, long *buf,
		      const REAL_VALUE_TYPE *r)
{
  unsigned long image, sig, exp;
  unsigned long sign = r->sign;
...
static void
decode_oprecomp_half (const struct real_format *fmt, REAL_VALUE_TYPE *r,
		      const long *buf)
{
  unsigned long image = buf[0] & 0xffff;
  bool sign = (image >> 15) & 1;
...
const struct real_format riscv_oprecomp_half_format =
  {
    encode_oprecomp_half,
    decode_oprecomp_half,
    2,
    8,
    8,
    -125,
